...
**Choix des couleurs :**
La couleur bleue a été choisie pour les éléments visuels (barres, boxplots, courbes) car c'est à cette couleur que l'on imagine lorsqu'on pense à un diamant.
Beaucoup de bleu car ça rappel la couleur du diamant  

**Boxplots :**
Les boxplots ont été sélectionnés pour comparer le prix selon des caractéristiques esthétiques telles que la coupe, la couleur et la clarté, à carat constant (~1). Ce type de graphique est particulièrement efficace pour visualiser la dispersion des prix tout en voyant les valeurs extrêmes.

**Histogramme :**
Nous avons utilisé un histogramme pour l’exploration initiale des prix. Il nous a permis d’identifier rapidement la forte concentration de diamants dans les tranches de prix les plus basses.

**Courbe (line plot) :**
Nous avons utilisé un line plot pour étudier la relation entre carat et prix moyen. Il nous a permit de visualiser l'évolution du prix moyen en fonction du poids du diamant et de mettre en lumiere la forte corelation entre prix et carat.

**Diagramme en barres (geom_bar) :**
Nous avons utilisé un geom bar pour analyser la fréquence des carats dans notre dataset. Il nous a permis de détecter un déséquilibre dans la distribution des observations au niveau du carat, ce qui nous a aidé à interpréter le line plot précédent.
...
```
Ce graphe nous montre donc que notre base de données a une majorité d'observations pour des petits carats et que l'intervalle entre 2.1 et 4.1 carats ne contient que très peu d'observations. Ce manque de données justifie probablement le résultat innatendue sur le graphe précédent.
...
ggplot(diamonds_filtered, aes(x = cut, y = price)) +
  ...
  theme_minimal(base_size = 15)
```
Ce boxplot nous montre que la qualité de coupe n'a pas un réel impact sur le prix du diamant bien que les qualités "Fair" soient globalement vendu moins cher que les autres. La qualité de coupe ne détermine donc pas le prix.
